...
def log_into_account():
    ...
    if not account:
        error_msg(f"Couldn't find an account with email {email}")
    ...
    for idx, c in enumerate(cages):
        ...
        for b in c.bookings:
            print('      * Booking: {}, {} days, booked? {}'.format(
                b.check_in_date if hasattr(b, 'check_in_date') else None,
                (b.check_out_date - b.check_in_date).days
                if hasattr(b, 'check_in_date') and hasattr(b, 'check_out_date')
                else None,
